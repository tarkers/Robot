...
class Musicsearch:
    ...
    def __init__(self):
        ...
        self.decodecheck = {"path":os.getcwd()+"\static\js\key.js", "key":"BASEKEY"}
        self.checkcount = 0
...
# ----------------------- get video/audio and next song------------------------
    def get_link(self):
        ...
        cleanr = re.compile('<.*?>|;')
        cleantext = re.sub(cleanr, '', target).replace("var ytInitialPlayerResponse = ", "")
        ...
        isvideo = 'https://i.ytimg.com/' in self.nowsong['pic']
        audio = formats[-2]
        video = formats[1]
        
        if "url" in audio:
            self.audiolink = audio['url']
            if isvideo:
                self.videolink = video['url']
        else:   
            with open(self.decodecheck["path"]) as f:
                funcstring = f.read()
                f.close()
                self.audiolink = self.link_combination(execjs.compile(funcstring),audio) 
            try:
                urllib.request.urlopen(self.audiolink)
                ...
                with open(self.decodecheck["path"]) as f:
                    ...
                    self.audiolink = self.link_combination(execjs.compile(funcstring),audio)      
            if isvideo:
                self.videolink = self.link_combination(execjs.compile(funcstring),video)
                try:
                    urllib.request.urlopen(self.videolink)
                except urllib.error.HTTPError:
                    self.videolink=""
        return True
    ...
    def link_combination(self,sigfunc,src):   
        ...
        link= unquote(splitlink[2].replace("url=", "")) + "&sig=" + sigfunc.call(key, siglist)
        return link
 # ---------------------- search page from the speakdata--------------------------

    def id_search(self, d):
        self.nowsong = Musicsearch.form_song_dict(d.sname, d.cname, d.aname, d.sid, d.cid, d.aid, d.pic)
        return self.get_link()

    @classmethod
    def select_best(cls,hot_has_id,speakdata,choice):
        if not hot_has_id:
            best_data=choice['歌曲']
        else:
            ...
            if "版" == ch or en in['lyrics','live','MV'] or 'googleusercontent' in choice['熱門搜尋結果']['pic']:
                ...
                songcheck = re.sub(r"\(.*?\)", "", choice['歌曲']['sname']).lower() 
                try:
                ...
                taiwanese = translator.translate(songcheck, lang_tgt='zh-TW').lower().strip()
                english = translator.translate(songcheck, lang_tgt='en').lower().strip()

                #langoption append
                langoption.append(translator.translate(songcheck, lang_tgt=results).strip())
                ...
                print(speaklower, langoption, cnamelower, songcheck, taiwanese, hot_low_mix, choice['熱門搜尋結果']['cid'] == choice['歌曲']['cid'] or cnamelower in hot_low_mix,
                      speaklower in langoption or songcheck in hot_low_mix or taiwanese in hot_low_mix)
                print(choice['歌曲']['sname'], choice['熱門搜尋結果']['sname'])
                # test code
    ...
    def page_search(self, speakdata):
        best_data = None
        hot_has_id = True  # assume hot place is song
        choice = {}
        url = "https://music.youtube.com/search?q=" + speakdata.replace(" ", "+")
        print('page_search: ', url)

        content = Musicsearch.music_renderer(url, "path: '\/search',")
        dataget = jsonpath(json.loads(content.replace("\\\\", "\\")), "$..musicShelfRenderer")
        for data in dataget:
            #six part
            ...
            if nameget in ["演出者", "專輯", "播放清單","社群播放清單"]:
                continue
            videoid = jsonpath(data, "$..playNavigationEndpoint.watchEndpoint[videoId]")
            ...
            title = jsonpath(data, "$..accessibilityPlayData.accessibilityData.label")[0]
            # print(title)
            ...
            pid = jsonpath( data, "$..playNavigationEndpoint[watchEndpoint][playlistId]")
            if pid != False:
                pid=pid[0]
            else:
                pid=""
            choice[nameget] = Musicsearch.form_song_dict(title[0],title[1],aname,videoid,cid,aid,pic,pid)
            print(aname,"----aname----")
        
        #choose the best data
        print(hot_has_id,speakdata,choice)
        best_data=Musicsearch.select_best(hot_has_id,speakdata,choice)
        ...
        if 'googleusercontent' in best_data['pic']:
            ...
            if data == None:
                ...
                if artist == None:
                    ...
                    content = Musicsearch.music_renderer(url, "path: '\/browse',")
                    if content == None:
                        print('no content',artist)
                        
                    content = json.loads(content.replace("\\\\", "\\"))
                    dataget = jsonpath(content, "$..thumbnail.thumbnails[0].url")
                    if dataget == False:
                        print("----------youtube does not  have this artist----------")
                        best_data['aid'] = best_data['cid']
                        best_data['cid'] = 'none'
                    else:
                        # add new artist
                        piclink = dataget[-1]
                        newartist = Artist(best_data['cid'], best_data['cname'], piclink)
                        newartist.add_artist()
                        print( piclink, best_data['cname'] ,"-------add new artist-------")
                #add the new song
    ...
    def get_songqueue(self):
        ...
        if self.pid != "":
            print("https://music.youtube.com/watch?v=" + self.nowsong['sid']+"&list="+self.pid, "\npid url")
            url = "https://music.youtube.com/watch?v=" +self.nowsong['sid']+"&list="+self.pid    
        print(url, "-------song queue---------------")
        driver.get(url)
        wait = WebDriverWait(driver, 30)
        element = wait.until(EC.visibility_of_all_elements_located((By.XPATH, '//*[@id="tab-renderer"]')))
        randomicon = driver.find_element_by_xpath('//*[@id="right-controls"]/div/tp-yt-paper-icon-button[3]')
        randomicon.click()
        time.sleep(0.5)
        ...
        queue = soup.findAll('ytmusic-player-queue-item', class_="style-scope ytmusic-player-queue")
        isvideo = False
        wholedata = []
        print(len(queue))
        for item in queue:
            ...
            data =Musicsearch.form_song_dict(test[0]['title'],test[1]['title'],"","","","",imglink,self.pid)
        ...
        for da in wholedata[1:]:
            ...
            if not isvideo:  # music
                ...
                if data == None:  
                    ...
                    namesearch=(da['cname']+"+"+re.sub(r'\(.*?\)', '', da['sname'])).replace(" ", "+")
                    ...
                    for inform in dataget:
                        found = False
                        sids = jsonpath(inform, "$..playNavigationEndpoint.watchEndpoint[videoId]")
                        if sids == False:
                            continue
                        for i in range(len(sids)):
                            if sids[i] == da['sid']:
                                ...
                                data = Song( da['cid'], da['cname'], da['sid'], da['sname'], da['pic'], da['aid'], da['aname'])
                                data.add_song()
                                print(da, "--------nextsong update the song-----------")
                                break
                        if found:
                            break
                elif data.pic == "default":
                    Song.updatepic(data.aid, da['pic'])
                    print(data.sname, data.cname, data.aname,'----------update pic-----------')
            if data != None:
                self.songlist.append(data)
            else:
                self.songlist.append(Song(da['cid'], da['cname'], da['sid'], da['sname'], da['pic'], da['aid'], da['aname']))
        playicon = driver.find_element_by_xpath('//*[@id="play-pause-button"]')
        if playicon.get_attribute("title") == "暫停":
            print("---still playing----")
            playicon.click()
    ...
    def songrequire(self, speak_words):
        isnext = False
        data = None
        # play the next song
        if "Next" in speak_words and len(self.songlist)>0:
             data = self.songlist.pop()
        # play the whole artist
        elif "的歌" in speak_words[-3:]:
            artist = re.sub(r"的歌.?", "", speak_words)
            artist_songs = Song.get_singer_song(artist)
        ...
        if data != None:
            ...
            if not noerror:
                ...
                if isnext and len(self.songlist) > 0:
                    ...
                    self.page_search(self.nowsong['cname']+" "+self.nowsong['sname'])
            print(noerror)
        else:
            noerror = self.page_search(speak_words)  # no id
            song = Musicsearch.get_song(self.nowsong)
            print(noerror)
        song['musicurl'] = self.audiolink
        song['videourl'] = self.videolink
        if 'googleusercontent' in song['image']:
            song['image'] = re.sub(
                r'=w(.*?)-h(.*?)-', '=w600-h600-', song['image'])
    ...
    def get_song(cls, data):
        ...
        if type(data) == dict:
            ...
            if data.pic == 'default':
                song['image'] = Artist.check_data(data.cid).pic
            else:
                song['image'] = data.pic
            song['album'] = data.aname
            song['artist'] = data.cname
